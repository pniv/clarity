...
export function getPointerCoordinates(event: MouseEvent | any) {
  const touch = (event.touches && event.touches[0]) || (event.pointerType && event.pointerType === 'touch' && event);
  const x: number = (touch || event).clientX;
  const y: number = (touch || event).clientY;
  return { x, y };
}
