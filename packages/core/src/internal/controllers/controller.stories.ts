  ...
  class DemoDraggableListController extends LitElement {
    ...
    private sortList(e: any) {
      if (e.detail.type === 'reordered') {
        const fromIndex = this.items.findIndex(i => `${i.id}` === e.detail.from.id);
        const targetIndex = this.items.findIndex(i => `${i.id}` === e.detail.target.id);
        this.items = insertBefore(fromIndex, targetIndex, this.items);
        e.preventDefault();
      }
    }
  }
