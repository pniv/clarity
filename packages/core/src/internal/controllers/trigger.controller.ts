...
export function triggerable<T extends Triggerable>(config = { focus: true }): ClassDecorator {
  return (target: any) => target.addInitializer((instance: T) => new TriggerRefController(instance, config));
}

export class TriggerRefController<T extends Triggerable> implements ReactiveController {
